...
def _build_title(
    ...
    metric_text = "instructions" if metric_label == "callgrind_instructions" else "wallclock"
...
def _build_body(
    *,
    ...
    if survivors:
        ...
        metric_text = "Callgrind instruction count" if metric_name == "callgrind_instructions" else "median wallclock"
        modules = sorted({_module_from_path(p) for it in survivors for p in _diff_paths(it.diff)})
    ...
    lines.append(f"Extraction harness: `{rel_extraction or extraction.extraction_path}`")
    if extraction.original_nodeid:
    ...
    lines.append("## Notes")
    lines.append("")
    lines.append(f"Generated by Spinwright v{spinwright.__version__} using {reasoning_model}.")
...
def _measurements_table(loop_result: LoopResult, metric_name: str) -> str:
    ...
    if b.callgrind is not None and fcg is not None:
        ...
        rows.append(("Callgrind instructions (per call)",
                     f"{base:,}", f"{final:,}", f"−{delta:.1%}"))
    if fwt is not None:
        base_med = b.walltime.median_seconds
        final_med = fwt.median_seconds
        delta = (base_med - final_med) / base_med if base_med > 0 else 0
        rows.append(("Wallclock median (µs)",
                     f"{base_med * 1e6:.2f}", f"{final_med * 1e6:.2f}", f"−{delta:.1%}"))
        rows.append(("Wallclock best (µs)",
                     f"{b.walltime.best_seconds * 1e6:.2f}",
                     f"{fwt.best_seconds * 1e6:.2f}", ""))
        rows.append(("Wallclock stddev (µs)",
